...
Status ColumnRow::RunGC(google::bigtable::admin::v2::GcRule const& gc_rule) {
  switch (gc_rule.rule_case()) {
    ...
    case google::bigtable::admin::v2::GcRule::kMaxAge: {
      return ApplyGCRuleMaxAge(gc_rule.max_age());
    }
...
StatusOr<bool> TimestampIsOlderThan(std::chrono::milliseconds timestamp,
  ...
  if (max_age_ms < std::chrono::milliseconds(1)) {
    return InvalidArgumentError(
        "max_age must be at least 1ms",
        GCP_ERROR_INFO().WithMetadata(
            "max age in ms", absl::StrFormat("%d", max_age_ms.count())));
  }
...
Status ColumnRow::ApplyGCRuleMaxAge(protobuf::Duration const& max_age) {
  std::chrono::milliseconds max_age_ms(
      protobuf::util::TimeUtil::DurationToMilliseconds(max_age));
  // The proto definitions require max_age to be at least 1ms.
  if (max_age_ms < std::chrono::milliseconds(1)) {
...
Status ColumnFamily::RunGC() {
  if (!gc_rule_.has_value()) {
    return Status();
  }

  for (auto it = rows_.begin(); it != rows_.end();) {
    auto status = it->second.RunGC(gc_rule_.value());
    if (!status.ok()) {
      return status;
    }
