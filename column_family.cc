...
void ColumnRow::ApplyGCRuleMaxAge(protobuf::Duration const& max_age) {
  auto now = std::chrono::duration_cast<std::chrono::milliseconds>(
      std::chrono::system_clock::now().time_since_epoch());
  auto not_before = now - std::chrono::duration_cast<std::chrono::milliseconds>(
                              std::chrono::seconds(max_age.seconds()));
  not_before =
      not_before - std::chrono::duration_cast<std::chrono::milliseconds>(
                       std::chrono::nanoseconds(max_age.nanos()));
  // Iterate from the back since the oldest cells are at the back
  // since we order the biggest (most recent) timestamps first.
  for (auto it = cells_.rbegin(); it != cells_.rend();) {
    if (it->first < not_before) {
      auto next_it = std::next(it);
      cells_.erase(std::prev(it.base()));
      it = next_it;
      continue;
    }

    // The cell is new enough and furthermore we can find no older
    // cell since they are stored in reverse chronological order of
    // timestamp
    return;
  }
}
...
StatusOr<bool> ColumnRow::GCRuleEraseVerdict(
  ...
  switch (rule.rule_case()) {
    case google::bigtable::admin::v2::GcRule::kMaxAge: {
      if (TimestampIsOlderThan(it->first, rule.max_age())) {
        return true;
      }
      return false;
      break;
...
void ColumnRow::ApplyGCRuleUnion(
    protobuf::RepeatedPtrField<google::bigtable::admin::v2::GcRule> const&
    /*rules*/) {}
...
StatusOr<std::shared_ptr<ColumnFamily>> ColumnFamily::ConstructColumnFamily(
  ...
  if (maybe_gc_rule.has_value()) {
    ...
    std::size_t gc_rule_size = gc_rule.ByteSizeLong();
    if (gc_rule_size > 500) {
      return InvalidArgumentError(
          "Supplied GcRule is too large: It must not exceed 500 bytes (when "
          "serialized)",
          GCP_ERROR_INFO().WithMetadata("GcRule", gc_rule.DebugString()));
    }
